...
def calculate_batch_status(df):
    ...
    # 進捗率計算
    progress_percentage = (approved_count + rejected_count) / total_items * 100
...
def export_to_excel_with_sheets(df, output_path=None):
    """
    グループA/B/Cを別シートに振り分けてExcelファイル出力
    """
    if output_path is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = f"asin_results_{timestamp}.xlsx"
    ...
    with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
        ...
        summary_data = pd.DataFrame({
            ...
            '件数': [
                len(df[df['confidence_group'] == 'A']), 
                len(df[df['confidence_group'] == 'B']), 
                len(df[df['confidence_group'] == 'C']), 
                len(df)
            ],
            '割合': [
                f"{len(df[df['confidence_group'] == 'A'])/len(df)*100:.1f}%" if len(df) > 0 else "0%", 
                f"{len(df[df['confidence_group'] == 'B'])/len(df)*100:.1f}%" if len(df) > 0 else "0%", 
                f"{len(df[df['confidence_group'] == 'C'])/len(df)*100:.1f}%" if len(df) > 0 else "0%",
                "100%"
            ]
        })
        ...
        # 各グループのシート
        df[df['confidence_group'] == 'A'].to_excel(
            writer, sheet_name='A_確実に使えるASIN', index=False)
        df[df['confidence_group'] == 'B'].to_excel(
            writer, sheet_name='B_要確認ASIN', index=False)
        df[df['confidence_group'] == 'C'].to_excel(
            writer, sheet_name='C_参考情報', index=False)
